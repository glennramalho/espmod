...
void setup(void) {
  ...
  if (! versiondata) {
    ...
    /* In case we find an error, we stop. In the model, we quit with an error
     * message as there is no point to simulate further. For the real version
     * we can go into a dead loop.
     */
#ifdef SYSCMOD
    SC_REPORT_ERROR("INO", "Failed to find PN532");
#else
    while (1); // halt
#endif
  }
