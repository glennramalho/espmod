/*******************************************************************************
 * esp32adc1.cpp -- Copyright 2019 (c) Glenn Ramalho - RFIDo Design
 *******************************************************************************
 * Description:
...
void esp32adc1::soc(int channel) {
...
int esp32adc1::get_width() { return width; }

void esp32adc1::set_width(int _w) {
   if (_w < 9 || _w > 12)
      PRINTF_WARN("ADC1", "Attempting to set illegal width %d", _w);
   if (busy()) {
      PRINTF_WARN("ADC1",
...
adc_atten_t esp32adc1::get_atten(int channel) {
   if (channel < 0 || channel > 7)
      PRINTF_WARN("ADC1",
...
void esp32adc1::set_atten(int channel, adc_atten_t _a) {
   if (channel < 0 || channel > 7)
      PRINTF_WARN("ADC1",
         "Attempting to set attenuation of illegal channel %d", channel);
...
void esp32adc1::switchon(bool on) {
   corrupted = true;
