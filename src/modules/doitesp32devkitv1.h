...
SC_MODULE(doitesp32devkitv1) {
   ...
   protected:
   /* If the simulation uses the flash for data storage, the ESP library needs
    * to intialize the partition table. For those cases, this needs to be set
    * and it will be called automatically before starting to simulate the
    * firmware.
    */
   bool flash_init;
   public:
   void set_flashinit(bool _v) { flash_init = _v; }
