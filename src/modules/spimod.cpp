...
void spimod::update_th() {
   while(true) {
      ...
      if (lowerusrbit_ev.triggered()) {
         spistruct->cmd.usr = 0;
         SETFIELD(cmd, SPI_USR_M, 0, SPI_USR_S);
      }

      /* In other cases, we take in the updated bits. */
      if (update_ev.triggered()) {
         /* Most registers we simply copy in. This is useful for debugging
          * issues.
          */
         cmd.write(spistruct->cmd.val);
         addr.write(spistruct->addr);
...
void spimod::return_th() {
   ...
   while (true) {
      ...
      spistruct->slv_wr_status = slv_wr_status.read();
   }
}
...
void spimod::configure(spi_dev_t *_spistruct) {
   spistruct = _spistruct;
   if (spistruct == NULL)
      SC_REPORT_FATAL("SPIMOD", "failed to set SPI to struct NULL");
   memset(spistruct, 0, sizeof(spi_dev_t));
}

void spimod::start_of_simulation() {
   /* We spawn a thread for each channel and timer. */
   ctrl.write(0); ctrl2.write(0);
   clock.write(0);
   pin.write(0);
...
int spimod::calcnextbit(int bitpos, bool littleendian, bool msbfirst) {
   ...
   if (msbfirst) {
      /* If we are in the last bit in a byte, we need to know which byte we go
       * to next. For little endian, we just advance, but for bigendian we need
       * to do some jumping.
       */
      if ((bitpos & 0x7) == 0x0 && littleendian) bitpos = bitpos + 8;
      else if ((bitpos & 0x7) == 0x0 && (bitpos & 0x18) == 0x0)
         bitpos = bitpos + 63;
   ...
   /* For LSB first, we start from bit zero. */
      ...
      else if ((bitpos & 0x7) == 0x7 && (bitpos & 0x18) == 0x0)
         bitpos = bitpos + 48;
...
int spimod::converttoendian(bool littleendian, bool msbfirst, int pos) { 
   /* First we check the msb/lsb. For this we jump to the end of the selected
    * bit.
    */
...
int spimod::getdly(int mode) {
   bool ck_out_or_i_out =
      RDFIELD(user, SPI_CK_OUT_EDGE_M, SPI_CK_OUT_EDGE_S) == 0
      || RDFIELD(user, SPI_CK_I_EDGE_M, SPI_CK_I_EDGE_S) == 0;
   int dlyout;
...
void spimod::activatecs(bool withdelay) {
   ...
   if (withdelay && RDFIELD(user, SPI_CS_SETUP_M, SPI_CS_SETUP_S)) {
      wait(period * RDFIELD(ctrl2, SPI_SETUP_TIME_M, SPI_SETUP_TIME_S));
   }
}
...
void spimod::deactivatecs(bool withdelay) {
   ...
   if (withdelay && RDFIELD(user, SPI_CS_HOLD_M, SPI_CS_HOLD_S)) {
      wait(period * RDFIELD(ctrl2, SPI_HOLD_TIME_M, SPI_HOLD_TIME_S));
   }
...
void spimod::setupmaster() {
   ...
   if (RDFIELD(clock, SPI_CLK_EQU_SYSCLK_M, SPI_CLK_EQU_SYSCLK_S)) {
      /* The system clock for the SPI is always the APB clock. */
      period = clockpacer.get_apb_period();
      hightime = sc_time(clockpacer.get_apb_period()/2);
    ...
    * different registers. Note that there are many possible illegal
    * combinations. Firmware has to get it right.
    */
   dlywr = getdly(RDFIELD(ctrl2, SPI_MOSI_DELAY_MODE_M, SPI_MOSI_DELAY_MODE_S));
   dlyrd = getdly(RDFIELD(ctrl2, SPI_MISO_DELAY_MODE_M, SPI_MISO_DELAY_MODE_S));
   ...
   if (RDFIELD(user, SPI_USR_MOSI_M, SPI_USR_MOSI_S) == 0) {
      startbit = -1;
      lastbitrd = -1;
   }
   else {
      wrlittleendian =
         RDFIELD(user, SPI_WR_BYTE_ORDER_M, SPI_WR_BYTE_ORDER_S) == 0;
      wrmsbfirst = RDFIELD(ctrl, SPI_WR_BIT_ORDER_M, SPI_WR_BIT_ORDER_S) == 0;
      /* For the start bit, we guess for the LSB firsrt/little endian case and
       * then we adjust the value.
       */
   ...
   if (RDFIELD(user, SPI_USR_MISO_M, SPI_USR_MISO_S) == 0) {
      startbitrd = -1;
      lastbitrd = -1;
   }
   else {
      rdlittleendian =
         RDFIELD(user, SPI_RD_BYTE_ORDER_M, SPI_RD_BYTE_ORDER_S) == 0;
...
void spimod::transfer_th() {
   ...
   /* We start off setting bit to -1 as that indicates there is no transmission
    * in progress.
    */
   int bit = -1;
   int bitrd = -1;
   while(1) {
      /* If we are not enabled, we need to wait for a start. This only happens
       * on the beginning. We also accept reset and master change.
       */
      if (bit == -1 && bitrd == -1)
         wait(start_ev | reset_ev | master.value_changed_event());

      /* We now have to check for master or reset events. We treat both as
       * resets.
       */
      ...
      /******************** DELAY 0 *******************************************/
      /* We need to find out what bit we send. If we are in the pre-write phase,
       * we send zero. We need to keep track of where in the data to send we
       * are. We need to check that the pointer is inside the data to send. It
      ...
      /* We check the delaywrite mode to see if we send now or not. We do the
       * same for the sample.
       */
      ...
      if (master.read()) {
         clk_o.write(!actclk);
         wait(hightime, master.value_changed_event() | reset_ev);
      }
      else wait(clk_i.value_changed_event() | master.value_changed_event()
         | reset_ev);
      if (master.event() || reset_ev.triggered()) {
         setupmasterslave();
         bit = -1;
      ...
      if (master.read()) {
         clk_o.write(actclk);
         wait(lowtime, master.value_changed_event() | reset_ev);
      }
      else wait(clk_i.value_changed_event() | master.value_changed_event()
         | reset_ev);
      if (master.event() || reset_ev.triggered()) {
       ...
       * the next bit. Note that it depends on the MSB vs LSB, word and byte
       * order.
       */
      if (bit == lastbit) bit = -1;
      else bit = calcnextbit(bit, wrlittleendian, wrmsbfirst);
      if (bitrd == lastbitrd) bitrd = -1;
      else bitrd = calcnextbit(bitrd, rdlittleendian, rdmsbfirst);

      /* If both read and write went idle we need to lower the USR bit. If we
       * do not have the keepactive, we also deactivate the CS lines.
       */
      if (bit == -1 && bitrd == -1) {
         lowerusrbit_ev.notify();
