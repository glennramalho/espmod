...
private:
        ...
        size_t r_available()
        {
            ...
            int count;
            int res = espm_ioctl_r(_fd, FIONREAD, &count);
            if(res < 0) {
                _failed = true;
                return 0;
            }
        ...
        size_t fillBuffer()
        {
            ...
            int res =
               espm_recv(_fd, _buffer + _fill, _size - _fill, MSG_DONTWAIT);
            if(res < 0) {
...
{
    int sockfd = espm_socket(AF_INET, SOCK_STREAM, 0);
    if (sockfd < 0) {
        log_e("socket: %d", errno);
        return 0;
    }
    espm_fcntl( sockfd, F_SETFL, espm_fcntl(sockfd, F_GETFL, 0) | O_NONBLOCK );
    ...
    espm_connect_r(sockfd, (struct sockaddr*)&serveraddr, sizeof(serveraddr));
    int res = espm_select(sockfd + 1, nullptr, &fdset, nullptr, timeout<0 ? nullptr : &tv);
    if (res != 1)
    {
...
{
    int res = espm_setsockopt(fd(), SOL_SOCKET, option, value, len);
    if(res < 0) {
...
{
    int res = espm_setsockopt(fd(), IPPROTO_TCP, option, (char *) value, sizeof(int));
    if(res < 0) {
...
int WiFiClient::getOption(int option, int *value)
{
    ...
    int res = espm_getsockopt(fd(), IPPROTO_TCP, option, (char *)value, &size);
    if(res < 0) {
        log_e("fail on fd %d, errno: %d, \"%s\"", fd(), errno, strerror(errno));
    }
    return res;
...
size_t WiFiClient::write(const uint8_t *buf, size_t size)
{
    ...
    while(retry) {
        ...
        if(espm_select(socketFileDescriptor + 1, NULL, &set, NULL, &tv) < 0) {
            return 0;
        }

        if(FD_ISSET(socketFileDescriptor, &set)) {
            res = espm_send(socketFileDescriptor, (void*) buf, bytesRemaining,
                  MSG_DONTWAIT);
            if(res > 0) {
                totalBytesSent += res;
...
void WiFiClient::flush() {
    ...
    while(a){
        ...
        res = espm_recv(fd(), buf, toRead, MSG_DONTWAIT);
        if(res < 0) {
            log_e("fail on fd %d, errno: %d, \"%s\"", fd(), errno, strerror(errno));
            stop();
            break;
...
uint8_t WiFiClient::connected()
{
    if (_connected) {
        uint8_t dummy;
        int res = espm_recv(fd(), &dummy, 0, MSG_DONTWAIT);
...
IPAddress WiFiClient::remoteIP(int fd) const
{
    ...
    espm_getpeername(fd, (struct sockaddr*)&addr, &len);
...
IPAddress WiFiClient::localIP(int fd) const
{
    struct sockaddr_storage addr;
    socklen_t len = sizeof addr;
    espm_getsockname(fd, (struct sockaddr*)&addr, &len);
    struct sockaddr_in *s = (struct sockaddr_in *)&addr;
    return IPAddress((uint32_t)(s->sin_addr.s_addr));
}
