...
int WiFiGenericClass::statusbits = 0;
...
{
   SC_REPORT_INFO("WIFI", "DNS Found Callback");
   // if(ipaddr) {
...
{
    bool valid;
    /* We could have an IP address. We do the dummest way to find out.
     * If it looks like an IP, it must be one.
     */
    valid = aResult.fromString(aHostname);

    /* If it worked, we return success. */
    if (valid) return 1;

    /* If it is a name, we return a token. This is always the same value, so
     * that a predefined code can be used. And we return it.
     */
    aResult.fromString("127.210.0.55");
    return 1;
}
