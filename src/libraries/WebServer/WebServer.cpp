/*
  ...
  Modified 29 July 2019 by Glenn Ramalho (RFIDo Design)
    - removed the FS and MD5 code as they are currently not supported in the
      ESPMOD version.
*/
...
static String md5str(String &in){
  /* We put some random numbers. Later we can put in the real MD5 algorithm. */
  char out[33] = {0};
  sprintf(out, "%08x%08x%08x%08x", rand(), rand(), rand(), rand());
  return String(out);
}
...
void WebServer::sendContent_P(PGM_P content, size_t size) {
  ...
  if(_chunked) {
    char * chunkSize = (char *)malloc(11);
    if(chunkSize){
      sprintf(chunkSize, "%lx%s", size, footer);
      _currentClientWrite(chunkSize, strlen(chunkSize));
      free(chunkSize);
    }
  }
