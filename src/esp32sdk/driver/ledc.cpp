...
sc_semaphore ledc_spinlock("ledc_spinlock", 1);
...
static void ledc_ls_channel_update(ledc_mode_t speed_mode, ledc_channel_t channel_num)
{
    if (speed_mode == LEDC_LOW_SPEED_MODE) {
...
static esp_err_t ledc_duty_config(ledc_mode_t speed_mode, ledc_channel_t channel_num, int hpoint_val, int duty_val,
...
esp_err_t ledc_timer_rst(ledc_mode_t speed_mode, uint32_t timer_sel)
{
    ...
    update_ledc();
    ledc_ls_timer_update(speed_mode, (ledc_timer_t)timer_sel);
    portEXIT_CRITICAL(&ledc_spinlock);
    return ESP_OK;
}
...
esp_err_t ledc_timer_config(const ledc_timer_config_t* timer_conf)
{
    ...
    ledc_timer_set((ledc_mode_t)speed_mode, (ledc_timer_t)timer_num, div_param, duty_resolution, timer_clk_src);
    // reset timer
    ledc_timer_rst((ledc_mode_t)speed_mode, timer_num);
    return ret;
}
...
esp_err_t ledc_channel_config(const ledc_channel_config_t* ledc_conf)
{
    ...
    ledc_set_duty_with_hpoint((ledc_mode_t)speed_mode,
       (ledc_channel_t)ledc_channel, duty, hpoint);
    /*update duty settings*/
    ledc_update_duty((ledc_mode_t)speed_mode, (ledc_channel_t)ledc_channel);
    /*bind the channel with the timer*/
    ledc_bind_channel_timer((ledc_mode_t)speed_mode,
       (ledc_channel_t)ledc_channel, timer_select);
    /*set interrupt type*/
    ledc_enable_intr_type((ledc_mode_t)speed_mode,
       (ledc_channel_t)ledc_channel, (ledc_intr_type_t)intr_type);
...
{
    int timer_sel = LEDC.channel_group[speed_mode].channel[channel].conf0.timer_sel;
    uint32_t freq = ledc_get_freq(speed_mode, (ledc_timer_t)timer_sel);
    uint32_t duty_cur = LEDC.channel_group[speed_mode].channel[channel].duty_rd.duty_read >> LEDC_DUTY_DECIMAL_BIT_NUM;
...
void ledc_fade_func_uninstall()
{
    ...
    for (mode = 0; mode < LEDC_SPEED_MODE_MAX; mode++) {
        for (channel = 0; channel < LEDC_CHANNEL_MAX; channel++) {
            ledc_fade_channel_deinit((ledc_mode_t)mode, (ledc_channel_t)channel);
        }
    }
    return;
}
