...
sc_semaphore spinlock("intr_spinlock", 1);
...
static vector_desc_t *get_desc_for_int(int intno, int cpu)
{
    ...
    if (vd==NULL) {
        vector_desc_t *newvd=(vector_desc_t *)malloc(sizeof(vector_desc_t));
                                        ...
                                        void *arg, intr_handle_t *ret_handle)
{
    ...
    /* The real code checks to make sure the handler is in the correct address
     * range. We can't check the address of the handler like the real code does
     * as we put the code just anywhere. We then have to skip this test.
     */
    ...
    ret=(intr_handle_data_t *)malloc(sizeof(intr_handle_data_t));
    if (ret==NULL) return ESP_ERR_NO_MEM;

    portENTER_CRITICAL(&spinlock);
    /* We do not have the CPU ID so we just put something. */
    ...
    if (flags&ESP_INTR_FLAG_SHARED) {
        ...
        shared_vector_desc_t *sh_vec=
           (shared_vector_desc_t *)malloc(sizeof(shared_vector_desc_t));
        if (sh_vec==NULL) {
            portEXIT_CRITICAL(&spinlock);
            free(ret);
            return ESP_ERR_NO_MEM;
        ...
        espintrptr->assignfn(cpu, intr, shared_intr_isr, (void *)vd);
    } else {
        //Mark as unusable for other interrupt sources. This is ours now!
        vd->flags=VECDESC_FL_NONSHARED;
        if (handler) {
            espintrptr->assignfn(cpu, intr, handler, arg);
        }
        if (flags&ESP_INTR_FLAG_EDGE) espintrptr->set_edge(1 << intr, cpu);
        vd->source=source;
    }
...
esp_err_t esp_intr_free(intr_handle_t handle)
{
    ...
    if ((handle->vector_desc->flags&VECDESC_FL_NONSHARED) || free_shared_vector) {
        ...
        espintrptr->deassignfn(handle->vector_desc->cpu,
