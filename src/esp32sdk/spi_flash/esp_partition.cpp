/*******************************************************************************
 * esp_partition.cpp -- Copyright 2019 Glenn Ramalho - RFIDo Design
 *******************************************************************************
 * Description:
 *   This file ports the esp_partition flash functions for the ESP32 to the
 *   ESPMOD SystemC model. It was based off the functions from Espressif
 *   Systems.
 *******************************************************************************
...
bool esp_flash_encryption_enabled() { return false; }
...
static esp_err_t load_partitions()
{
   ...
   partition_list_item_t* item;

   item = new partition_list_item_t;
   item->info.address = 0x9000;
   item->info.size = 0x5000;
   item->info.type = ESP_PARTITION_TYPE_DATA;
   item->info.subtype = (esp_partition_subtype_t)ESP_PARTITION_SUBTYPE_DATA_NVS;
   item->info.encrypted = false; /* Not supported */
   strcpy(item->info.label, "nvs");
   SLIST_INSERT_HEAD(&s_partition_list, item, next);
   last = item;

   item = new partition_list_item_t;
   item->info.address = 0xe000;
   item->info.size = 0x2000;
   item->info.type = ESP_PARTITION_TYPE_DATA;
   item->info.subtype = (esp_partition_subtype_t)ESP_PARTITION_SUBTYPE_DATA_OTA;
   item->info.encrypted = false; /* Not supported */
   strcpy(item->info.label, "otadata");
   SLIST_INSERT_AFTER(last, item, next);
   last = item;

   item = new partition_list_item_t;
   item->info.address = 0x10000;
   item->info.size = 0x140000;
   item->info.type = ESP_PARTITION_TYPE_APP;
   item->info.subtype =(esp_partition_subtype_t)ESP_PARTITION_SUBTYPE_APP_OTA_0;
   item->info.encrypted = false; /* Not supported */
   strcpy(item->info.label, "ota_0");
   SLIST_INSERT_AFTER(last, item, next);
   last = item;
  
   item = new partition_list_item_t;
   item->info.address = 0x150000;
   item->info.size = 0x140000;
   item->info.type = ESP_PARTITION_TYPE_APP;
   item->info.subtype =(esp_partition_subtype_t)ESP_PARTITION_SUBTYPE_APP_OTA_1;
   item->info.encrypted = false; /* Not supported */
   strcpy(item->info.label, "ota_1");
   SLIST_INSERT_AFTER(last, item, next);
   last = item;

   item = new partition_list_item_t;
   item->info.address = 0x290000;
   item->info.size = 0x1000;
   item->info.type = ESP_PARTITION_TYPE_DATA;
   item->info.subtype =
      (esp_partition_subtype_t)ESP_PARTITION_SUBTYPE_DATA_NVS; /* TODO ??? */
   item->info.encrypted = false; /* Not supported */
   strcpy(item->info.label, "eeprom");
   SLIST_INSERT_AFTER(last, item, next);
   last = item;

   item = new partition_list_item_t;
   item->info.address = 0x291000;
   item->info.size = 0x16F000;
   item->info.type = ESP_PARTITION_TYPE_DATA;
   item->info.subtype =
      (esp_partition_subtype_t)ESP_PARTITION_SUBTYPE_DATA_SPIFFS;
   item->info.encrypted = false; /* Not supported */
   strcpy(item->info.label, "spiffs");
   SLIST_INSERT_AFTER(last, item, next);
   last = item;
...
{
    return ESP_ERR_NOT_SUPPORTED; /* Not supported */
}
